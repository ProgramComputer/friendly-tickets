      ...
      <Popover open={isStatusOpen} onOpenChange={setIsStatusOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(
              'h-8',
              (selectedStatus?.length ?? 0) > 0 && 'bg-muted text-muted-foreground'
            )}
          >
            <Filter className="mr-2 h-4 w-4" />
            Status
            {(selectedStatus?.length ?? 0) > 0 && ` (${selectedStatus.length})`}
          </Button>
        </PopoverTrigger>
      ...
      <Popover open={isPriorityOpen} onOpenChange={setIsPriorityOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(
              'h-8',
              (selectedPriority?.length ?? 0) > 0 && 'bg-muted text-muted-foreground'
            )}
          >
            <Filter className="mr-2 h-4 w-4" />
            Priority
            {(selectedPriority?.length ?? 0) > 0 && ` (${selectedPriority?.length})`}
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-48 p-2">
